...
import { Alert, FlatList, SectionList, Dimensions, GestureResponderEvent, Pressable, ScrollView, StyleProp, StyleSheet, Text, TextProps, View, ViewProps } from "react-native";
        ...
        props.forEach(grade => {
            return (
                ...
                <Pressable style={{ backgroundColor: 'red' }}>
                    <Text>{grade.grade} - {grade.date} - {grade.description}</Text>
                </Pressable>
            )
        })
    )
            ...
            */}
            ...
            <ScrollView>
                {data.map((value: ISubjects, key) => (
                    <Pressable key={key.toString()} style={styles.Node} onPress={()=>{navigation.navigate('Grades', {"grade": value.grades});}}
                        //onPress={() => { Grades(props.grades) }} 
                        android_ripple={{ color: "powderblue" }}>
                        <Text style={{ alignSelf: 'center' }}>{value.subjectName} - {value.teacher}</Text>
